...
namespace SIMSA.Pages
{
	...
	public partial class Menu : ContentPage
	{
		...
		public Menu(Config config, Action<Config> save)
		{
			...
			this.save = save;
			pages = ImmutableArray.Create<IConfigurable>(new Braille(config, new BrailleText()), new Morse(config, new MorseCode()), new Numeric(config, new NumericCode()), new Vigenere(config, new VigenereText()), new FlagSemaphore(config, new SemaphoreText()), new Playfair(config, new PlayfairCipher()), new Primes(config), new Settings(config, Save));
